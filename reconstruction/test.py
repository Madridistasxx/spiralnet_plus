...
from utils import DataLoader, mesh_sampling, utils, writer, mesh_sampling_no_psbody
...
# ddacs
parser.add_argument("--component", type=str, default="die")
...
# load dataset
print("Loading data...")
data_dir = "/mnt/data/datasets/ddacs/graph"
root = Path(data_dir).parent
datamodule = SurfaceDDACS(root)
datamodule.setup()
...
if not osp.exists(transform_fp):
    ...
    template_fp = osp.join(args.data_fp, 'template', 'template_die.obj')
    template_mesh = utils.obj_to_pyg_data(template_fp)
    ds_factors = [4, 4, 4, 4]
    _, A, D, U, F, V = mesh_sampling_no_psbody.generate_transform_matrices(
